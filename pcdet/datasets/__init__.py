...
def build_dataloader(dataset_cfg, class_names, batch_size, dist, root_path=None, workers=4,
    ...
    Returns
        ...
        sampler: 数据采样器
    """
    # 从字典中拿出一个数据类Dataset，并生成一个这个数据类的对象
    # 这其中也会配置数据增强的方法
    ...
    #merge_all_iters_to_one_epoch是什么操作
    if merge_all_iters_to_one_epoch:
        #hasattr(dataset, 'merge_all_iters_to_one_epoch')判断对象有没有对应属性。
        #assert断言是否正确，如果不正确则会报错。
        assert hasattr(dataset, 'merge_all_iters_to_one_epoch')
        dataset.merge_all_iters_to_one_epoch(merge=True, epochs=total_epochs)

    if dist:
        if training:
            #不使用DistributedSampler，每块卡上（每个进程）每个epoch中都迭代了所有的数据
            #使用DistributedSampler，一个epoch的数据会分散到多张卡上。
        ...
        sampler = None
    # 初始化DataLoader，（这只是取数据的迭代器）此时并没有进行数据采样和加载，只有在训练中才会按照batch size调用__getitem__加载数据
