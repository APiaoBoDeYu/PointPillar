...
class VoxelGeneratorWrapper():
    ...
    def generate(self, points):
        if self.spconv_ver == 1:
            ...
            voxel_output = self._voxel_generator.point_to_voxel(tv.from_numpy(points))#对voxel进行了随机抽取
...
class DataProcessor(object):
    ...
    def transform_points_to_voxels(self, data_dict=None, config=None):
        if data_dict is None:
            #设置体素配置
            #截取的点云范围是[-34.56, -39.68, -3.0, 34.56, 39.68, 3]
            #得到[69.12, 79.36, 4]/[0.16, 0.16, 6] = [432, 496, 1]
            #得到长宽高方向各能得到多少体素
        ...
        if self.voxel_generator is None:
            ...
            self.voxel_generator = VoxelGeneratorWrapper(
                vsize_xyz=config.VOXEL_SIZE,#给定每个pillar的大小  [0.16, 0.16, 6]
                coors_range_xyz=self.point_cloud_range,#给定点云的范围 [0, -39.68, -3, 69.12, 39.68, 1]
                num_point_features=self.num_point_features,#给定每个点云的特征维度，这里是x，y，z，r 其中r是激光雷达反射强度
                max_num_points_per_voxel=config.MAX_POINTS_PER_VOXEL,#给定每个pillar中最多能有多少个点 32
                max_num_voxels=config.MAX_NUMBER_OF_VOXELS[self.mode],#最多选取多少个pillar，因为生成的pillar中，很多都是没有点在里面的
            )
        ...
        #num_points ：代表了每个生成的pillar中有多少个有效的点维度是[m,]，因为不满32会被0填充。
        # 7279，32，4    7279，3           7279，1
        voxels, coordinates, num_points = voxel_output
